...
//localhost:3000/players/bar
app.get("/players/:playerName", (req, res) => {
  // render the correct player from the `players` array
  //   console.log(req.params);

  const singlePlayer = players.find(el => {
    if (el.name === req.params.playerName) {
      return true;
    }
  });

  res.render("player.hbs", { player: singlePlayer });
...
app.get("/search", (req, res) => {
  const playersList = players.filter(el => {
    if (el.team === req.query.q) return true;
  });

  res.render("players.hbs", { playersList: playersList });
});
